...
// 아바타 폴백 함수 (DB에 아바타가 없을 경우만 기본 이미지 생성)
...
if (typeof window !== 'undefined') {
  ...
  Object.keys(audioCache).forEach(key => {
    ...
    if (key === 'bgm' && currentMatchPhase === 'idle') {
      audio.muted = false; // 뮤트 해제
      audio.play().catch(() => {});
    } else {
      audio.muted = false;
      // 다른 소리들은 잠깐 재생 후 정지시켜서 브라우저 권한만 획득
...
function App() {
  ...
  const [entryLegend, setEntryLegend] = useState('방갈로르');
  const [entryWeapons, setEntryWeapons] = useState<string[]>(['VK-47 플랫라인', '햄록 버스트 AR']);
  ...
  // 🌟 실제 DB(Supabase)와 연동되는 상태 변수들
  const [logs, setLogs] = useState<any[]>([]); 
  const [rankers, setRankers] = useState<any[]>([]); // 들어온 순서대로 배치될 랭커 리스트
  ...
  useEffect(() => {
    ...
    ['hover', 'click', 'match_start', 'success', 'error'].forEach(key => {
      if (audioCache[key]) {
        audioCache[key].muted = !sfxEnabled;
        audioCache[key].volume = sfxVolume * (BASE_VOLUMES[key] || 1.0);
      }
    });
  ...
  useEffect(() => {
    ...
    if (matchPhase === 'idle') {
      stopSFX('waiting');
      if (globalBgmEnabled) playSFX('bgm'); 
    } else if (matchPhase === 'waiting') {
      stopSFX('bgm'); 
      if (globalBgmEnabled) playSFX('waiting'); 
  ...
  useEffect(() => {
    if (rankers.length > 0 && vfxEnabled) {
      ...
      if (prevTopPlayer.current && prevTopPlayer.current !== currentTop) {
        setShowComet(true);
        setTimeout(() => setShowComet(false), 3000);
      }
  ...
  const fetchRankers = async () => {
    // 가입한(데이터가 생성된) 순서대로(created_at 오름차순) 불러옵니다. 남철님이 1등!
    ...
    if (profiles && profiles.length > 0) {
      const processed = profiles.map((r, i) => ({
        ...
        favorite_legend: r.favorite_legend || '방갈로르',
        favorite_weapon: r.favorite_weapon || 'VK-47 플랫라인',
  ...
  const handleStartMatch = () => {
    ...
    if (!entryOpponent.trim()) {
      playSFX('error');
      return alert("상대방 닉네임을 정확히 입력하세요!");
    }
  ...
  const handleReportScore = async () => {
    if (myReportedScore === null) {
      playSFX('error');
      return alert("스코어를 선택하세요!");
    }
    if (!user) {
      playSFX('error');
      return alert("로그인이 필요합니다!");
    ...
    const { error } = await supabase.from('matches').insert([{
      ...
      score_left: myReportedScore, score_right: 3 - myReportedScore,
    ...
    if (!error) { 
      ...
      setEntryOpponent('');
      fetchData(); // 전적 갱신
      fetchRankers(); // 승패가 기록되었을 수 있으므로 랭커 갱신
    } else {
      playSFX('error');
  ...
  const renderCombatLogItem = (log: any, index: number) => {
    ...
    // DB 데이터를 기반으로 아바타를 가져옴
    const winnerAvatar = getAvatarFallback(winnerName, rankers);
    const loserAvatar = getAvatarFallback(loserName, rankers);

    const weapon1 = log.left_weapons?.[0] || ALL_WEAPONS[Math.floor(Math.random() * ALL_WEAPONS.length)];
    const weapon2 = log.left_weapons?.[1] || ALL_WEAPONS[Math.floor(Math.random() * ALL_WEAPONS.length)];
    const usedLegend = log.left_legend || ALL_LEGENDS[Math.floor(Math.random() * ALL_LEGENDS.length)];
    ...
    return (
      <div key={index} onMouseEnter={() => playSFX('hover')} className="bg-black/60 border border-cyan-500/30 p-5 rounded-[1.5rem] flex flex-col justify-center gap-3 hover:border-cyan-400/80 transition-all shadow-inner relative group combat-log-item cursor-default h-[140px] shrink-0">
        ...
        <div className="flex items-center justify-between w-full mt-2">
          <div className="flex items-center gap-3 flex-1 overflow-hidden pr-2">
             <div className="relative shrink-0">
               <span className="absolute -top-1.5 -left-1.5 bg-yellow-400 text-black text-[8px] font-black px-1 rounded shadow-[0_0_5px_gold] z-10">WIN</span>
               <img src={winnerAvatar} className={`w-10 h-10 rounded-full border-2 ${isLeftWinner ? 'border-yellow-400 shadow-[0_0_8px_gold]' : 'border-slate-500'} bg-black`} alt="winner"/>
             </div>
             <span className="font-black text-white text-xl truncate drop-shadow-[0_0_5px_white] nickname">{winnerName}</span>
             
             <span className="text-[11px] text-slate-500 font-black italic mx-2 shrink-0">VS</span>
             
             <img src={loserAvatar} className={`w-8 h-8 rounded-full border-2 border-slate-600 opacity-60 shrink-0 bg-black`} alt="loser"/>
             <span className="font-bold text-slate-400 text-base truncate">{loserName}</span>
          </div>

          <div className="text-3xl font-black tracking-widest text-white shrink-0 ml-2 drop-shadow-[0_0_10px_white]">
             <span className="text-yellow-400">{winnerScore}</span>
             <span className="text-slate-600 mx-1">:</span>
             <span className="text-slate-400">{loserScore}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs font-black bg-white/5 px-4 py-2 rounded-xl w-fit shadow-inner border border-white/10 details mt-1">
           <span className="text-pink-400">[{usedLegend}]</span>
           <span className="text-cyan-300">{weapon1}</span>
           <span className="text-slate-500">/</span>
           <span className="text-cyan-300">{weapon2}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="flex h-screen bg-black text-slate-300 font-sans overflow-hidden relative select-none">
      ...
      {showComet && vfxEnabled && (
        <div className="fixed inset-0 z-[100] pointer-events-none overflow-hidden">
           <div className="comet-path"><div className="comet-head shadow-[0_0_50px_#fff]"></div><div className="comet-tail"></div></div>
        </div>
      )}
      ...
      <aside className="w-20 bg-black/20 backdrop-blur-md border-r border-cyan-500/30 shadow-[0_0_30px_rgba(34,211,238,0.15)] flex flex-col items-center py-10 gap-10 z-20 shrink-0 h-screen fixed left-0">
        <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center border border-white/10 shadow-[0_0_20px_rgba(34,211,238,0.4)] cursor-pointer" onMouseEnter={() => playSFX('hover')} onClick={() => playSFX('click')}>
          <Star className="text-cyan-400 fill-cyan-400 animate-pulse" size={24}/>
        </div>
          ...
          {MENU_ITEMS.map((item) => (
            <div 
              ...
              className={`cursor-pointer transition-all duration-300 ${
                activeMenu === item.id 
                ? 'text-cyan-400 drop-shadow-[0_0_10px_cyan] scale-110' 
                : 'hover:text-slate-300 hover:scale-105'
              }`}
            >
              <item.icon size={24}/>
            </div>
      ...
      <div className="flex-1 flex flex-col z-10 relative ml-20 h-screen overflow-y-auto custom-scrollbar">
        ...
        <header className="px-10 py-8 flex justify-between items-center shrink-0 border-b border-cyan-500/30 bg-black/20 backdrop-blur-md shadow-[0_0_30px_rgba(34,211,238,0.15)]">
          <div className="flex items-center gap-6 cursor-pointer" onMouseEnter={() => playSFX('hover')} onClick={() => playSFX('click')}>
            <h1 className="text-6xl font-black text-white italic tracking-tighter uppercase drop-shadow-[0_0_20px_rgba(192,132,252,0.8)]">은하단</h1>
            <div className="h-12 w-[3px] bg-gradient-to-b from-cyan-400 to-purple-400 mx-3 shadow-[0_0_10px_cyan]"></div>
            <div className="flex flex-col justify-center">
               <p className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 uppercase tracking-widest drop-shadow-[0_0_8px_rgba(34,211,238,0.4)]">별의 전쟁 : 시즌 1</p>
          ...
          {user ? (
            <div onMouseEnter={() => playSFX('hover')} onClick={handleLogout} className="flex items-center gap-4 bg-black/60 p-2 rounded-full border border-white/10 pr-8 shadow-2xl border-l-cyan-500 border-l-4 cursor-pointer hover:border-l-pink-500 transition-all group">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[9px] font-bold text-cyan-400 group-hover:text-pink-400 uppercase tracking-tighter">Click to Logout</span>
              </div>
            </div>
          ) : (
            <button onMouseEnter={() => playSFX('hover')} onClick={handleLogin} className="flex items-center gap-3 bg-cyan-600/20 border-2 border-cyan-400 py-2.5 px-8 rounded-full shadow-[0_0_20px_cyan] hover:bg-cyan-400 hover:text-black transition-all font-black text-sm uppercase tracking-widest cursor-pointer">
              <LogIn size={20}/> DISCORD LOGIN
            </button>
          )}
        </header>
        ...
        {activeMenu === 'home' && (
          <main className="flex-1 p-10 grid grid-cols-12 gap-8 items-stretch pb-20 animate-in fade-in duration-500 min-h-[1400px]">
            ...
            <div className="col-span-12 xl:col-span-3 flex flex-col gap-8 h-full relative">
                      ...
                      <div className="space-y-3 flex flex-col relative pb-1">
                          ...
                          <div className="bg-black/60 p-2.5 rounded-2xl border border-white/5 shadow-inner shrink-0 relative z-20">
                             ...
                             <input 
                                ...
                                placeholder="상대방 닉네임" 
                                className="w-full bg-transparent outline-none text-white font-black text-lg px-1 select-text cursor-text" 
                             />
                          </div>
                          
                          {entryMode === 'free' && (
                            <div className="space-y-2 flex flex-col relative z-10 overflow-hidden">
                               <select onMouseEnter={() => playSFX('hover')} value={entryLegend} onChange={(e) => { playSFX('click'); setEntryLegend(e.target.value); }} className="w-full bg-black/60 border border-white/10 p-2.5 rounded-xl text-sm font-black text-white outline-none cursor-pointer shrink-0 select-text">
                               ...
                               <div className="flex flex-col gap-2 overflow-hidden relative">
                                  {[0, 1].map(i => (
                                    <select key={i} onMouseEnter={() => playSFX('hover')} value={entryWeapons[i]} onChange={(e) => {const w = [...entryWeapons]; w[i] = e.target.value; setEntryWeapons(w); playSFX('click');}} className="w-full bg-black/60 border border-white/10 p-2.5 rounded-xl text-sm font-black text-pink-400 outline-none cursor-pointer shrink-0 select-text">
                          ...
                          <button 
                            ...
                            className="w-full py-4 rounded-[1.5rem] font-black text-lg text-white bg-blue-600 shadow-[0_0_20px_rgba(37,99,235,0.4)] hover:bg-blue-500 transition-all border border-white/10 shrink-0 relative z-20 mt-1 cursor-pointer"
                          >
                            대전 생성 (Entry)
                          </button>
                      </div>
               ...
               {/* 실제 가입한 랭커(rankers) 데이터를 기반으로 접속 현황 노출 */}
               <section className="bg-black/50 backdrop-blur-2xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[2.5rem] p-4 flex flex-col overflow-hidden h-[880px] shrink-0">
                  ...
                  <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-1 pb-1">
                     {rankers.length > 0 ? rankers.slice(0, 10).map((ou, i) => {
                        ...
                        const targetUserRankNum = ou.rankIndex + 1; 
                        const isChallengeableByRank = targetUserRankNum < currentUserRankNum; 
                          ...
                          <div key={i} onMouseEnter={() => playSFX('hover')} className="bg-black/60 border border-white/10 p-3 rounded-xl flex items-center justify-between hover:border-cyan-400/50 transition-all group cursor-default">
                             <div className="flex items-center gap-3">
                                <div className="relative">
                                   <img src={ou.avatar_url} className="w-9 h-9 rounded-full border border-white/20" alt="profile"/>
                                   <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-black rounded-full"></div>
                                </div>
                                <div className="flex flex-col">
                                   <span className="text-sm font-black text-white truncate max-w-[90px]">{ou.display_name}</span>
                                   <span className={`text-[9px] font-black uppercase tracking-widest ${rankInfo.color}`}>{rankInfo.title} {rankInfo.num}</span>
                                </div>
                             </div>
                             {ou.is_challengeable !== false ? (
                                <button 
                                  ...
                                  className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all shadow-[0_0_10px_rgba(34,211,238,0.2)] cursor-pointer ${
                                    isChallengeableByRank 
                                      ? 'bg-cyan-600/20 border border-cyan-500/50 text-cyan-400 hover:bg-cyan-500 hover:text-black' 
                                ...
                                <span className="px-3 py-1.5 bg-slate-800/50 border border-slate-700 text-slate-500 text-[9px] font-black rounded-xl uppercase tracking-widest cursor-default">
                                  매치 중
                                </span>
                             )}
                          </div>
            ...
            <div className="col-span-12 xl:col-span-5 flex flex-col gap-8 h-full relative">
                  ...
                  <div className="flex flex-col relative z-10">
                      ...
                      {matchPhase === 'waiting' ? (
                        <div className="flex flex-col items-center justify-center opacity-90 relative z-10 py-16 animate-in fade-in">
                           ...
                           <button onMouseEnter={() => playSFX('hover')} onClick={handleCancelMatch} className="mt-10 px-10 py-3 rounded-full border-2 border-pink-500/50 text-pink-400 font-black hover:bg-pink-500 hover:text-white transition-all shadow-[0_0_15px_rgba(236,72,153,0.3)] cursor-pointer">
                              매칭 취소 (Cancel)
                           </button>
                        </div>
                        ...
                        <div className="flex flex-col pt-1 pb-1 animate-in fade-in overflow-hidden relative z-10 gap-3 mt-1">
                           <div className={`p-6 rounded-[2.5rem] border-2 shadow-[0_0_40px_rgba(0,0,0,0.5)] ${activeMatch.mode === 'random' ? 'border-cyan-400/50 bg-cyan-400/5' : 'border-pink-400/50 bg-pink-400/5'} text-center relative overflow-hidden flex flex-col justify-center shrink-0 cursor-default`} onMouseEnter={() => playSFX('hover')}>
                              <h4 className="text-6xl font-black text-white tracking-tighter mb-6 drop-shadow-[0_0_20px_white] leading-tight shrink-0">
                                 {activeMatch.legend}
                              </h4>
                              <div className="flex flex-col gap-3 items-center shrink-0">
                                 {activeMatch.weapons.map((w:any, idx:number) => (
                                   <span key={idx} className={`px-6 py-2 rounded-2xl text-base font-black border-2 w-full max-w-[350px] shadow-inner ${activeMatch.mode === 'random' ? 'border-cyan-400/50 text-cyan-400' : 'border-pink-400/50 text-pink-400'}`}>
                                      {w}
                                   </span>
                                 ))}
                              </div>
                           ...
                           <div className="flex flex-col gap-3 pt-3 border-t border-white/5 shrink-0 relative z-20">
                              ...
                              <button 
                                ...
                                disabled={myReportedScore === null}
                                className={`w-full py-4 rounded-[1.5rem] font-black text-xl transition-all uppercase tracking-widest shrink-0 relative z-20 cursor-pointer ${
                                  myReportedScore !== null 
                                  ? 'text-cyan-400 border-2 border-cyan-400/50 shadow-[0_0_15px_rgba(34,211,238,0.3)] hover:bg-cyan-400 hover:text-black' 
                                  : 'text-slate-500 border-2 border-slate-700 bg-slate-800/50 cursor-not-allowed'
                                }`}
                              >
               ...
               <section className="bg-black/40 backdrop-blur-2xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[2.5rem] p-6 flex flex-col overflow-hidden h-[940px] shrink-0">
                  <h3 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 uppercase tracking-tight text-center w-full block mb-4 border-b border-white/5 pb-2 shrink-0 drop-shadow-[0_0_8px_cyan] cursor-default" onMouseEnter={() => playSFX('hover')}>
                     최근 전투 기록
                  </h3>
                  ...
                  <div className="px-2 pt-2 flex flex-col relative z-10">
                      <div className="flex flex-col items-start shrink-0 mb-4 mt-1">
                          <div className="flex items-center gap-6 mb-1 cursor-default" onMouseEnter={() => playSFX('hover')}>
                              ...
                              <div className="flex flex-col">
                                  <h3 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-pink-500 uppercase tracking-tighter drop-shadow-[0_0_10px_orange] mt-1 pt-1">
                                     은하단 랭킹
                                  </h3>
                                  <p className="text-sm font-black text-cyan-400 italic uppercase leading-tight pt-1 drop-shadow-[0_0_8px_cyan]">
                                     명예의 전당<br/><span className="text-xs text-slate-400">(HALL OF FAME)</span>
                                  </p>
                              </div>
                          </div>
                      </div>
                                ...
                                <div key={r.id} onMouseEnter={() => playSFX('hover')} onClick={() => { playSFX('click'); setSelectedPlayer({...r, rankIndex: r.rankIndex}); }} className={`mx-1 p-3 pt-8 rounded-[1.2rem] border transition-all cursor-pointer group hover:scale-[1.02] ${grandRank.glow} border-cyan-400/30 hover:border-cyan-400 bg-black/60 relative flex flex-col justify-center shrink-0 z-20 rank-item`}>
                                   ...
                                   <div className="flex items-center justify-between w-full mt-3 relative z-10 px-2">
                                      <div className="flex items-center gap-4 flex-1 overflow-hidden">
                                         ...
                                         <span className={`text-white font-black truncate group-hover:text-cyan-400 transition-colors tracking-tight whitespace-nowrap nickname ${r.rankIndex === 0 ? 'text-2xl' : 'text-xl'}`}>
                                             {r.display_name}
                                         </span>
                                      </div>
                                      ...
                                      <div className="flex flex-col items-end gap-1 shrink-0 ml-2">
                                         ...
                                         {r.rankIndex === 0 && (
                                           <span className={`font-black text-[10px] px-2 py-0.5 rounded bg-yellow-400/20 text-yellow-400 border border-yellow-400/50 shadow-[0_0_5px_gold]`}>
                                              👑 왕좌방어 스택: {r.defense_stack || 0}
                                           </span>
                                         )}
                                      </div>
                                   </div>
        ...
        {/* =====================================================================
            ⚙️ 환경설정 메뉴 (풀기능 작동 & CONYMON 워터마크 추가)
            ===================================================================== */}
        {activeMenu === 'settings' && (
          <main className="flex-1 p-10 h-full overflow-y-auto custom-scrollbar pb-20 animate-in fade-in duration-500">
            ...
            <div className="bg-black/50 backdrop-blur-2xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[3rem] p-12 max-w-3xl space-y-10">
               ...
               <div className="flex items-center gap-8 border-b border-white/10 pb-10 cursor-default" onMouseEnter={() => playSFX('hover')}>
                  ...
                  <div>
                     <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase">{currentUserName || "GUEST PILOT"}</h3>
               ...
               <div className="flex items-center justify-between border-b border-white/10 pb-8">
                 <div className="flex-1">
                   <div className="flex justify-between items-center mb-2">
                     <h4 className="text-xl font-black text-white tracking-widest cursor-default" onMouseEnter={() => playSFX('hover')}>배경음악 (BGM)</h4>
                     <div 
                       onClick={() => { playSFX('click'); setBgmEnabled(!bgmEnabled); }} 
                       className={`w-14 h-7 rounded-full relative cursor-pointer transition-all ${bgmEnabled ? 'bg-cyan-500 shadow-[0_0_15px_cyan]' : 'bg-white/20'}`}
                     >
                       <div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${bgmEnabled ? 'right-1' : 'left-1'}`}></div>
                     </div>
                   </div>
                   ...
                   <div className="flex items-center gap-4">
                      ...
                      <input 
                        ...
                        className="w-full bg-white/10 h-2 rounded-lg cursor-pointer volume-slider"
                        disabled={!bgmEnabled}
                        onMouseEnter={() => playSFX('hover')}
                      />
               ...
               <div className="flex items-center justify-between border-b border-white/10 pb-8">
                 <div className="flex-1">
                   <div className="flex justify-between items-center mb-2">
                     <h4 className="text-xl font-black text-white tracking-widest cursor-default" onMouseEnter={() => playSFX('hover')}>효과음 (SFX)</h4>
                     <div 
                       onClick={() => { playSFX('click'); setSfxEnabled(!sfxEnabled); }} 
                       className={`w-14 h-7 rounded-full relative cursor-pointer transition-all ${sfxEnabled ? 'bg-cyan-500 shadow-[0_0_15px_cyan]' : 'bg-white/20'}`}
                     >
                       <div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${sfxEnabled ? 'right-1' : 'left-1'}`}></div>
                     </div>
                   </div>
               ...
               <div className="flex items-center justify-between border-b border-white/10 pb-8">
                 <div className="flex-1">
                   <div className="flex justify-between items-center mb-2">
                     <h4 className="text-xl font-black text-white tracking-widest cursor-default" onMouseEnter={() => playSFX('hover')}>배경 애니메이션 (VFX)</h4>
                     <div 
                       onClick={() => { playSFX('click'); setVfxEnabled(!vfxEnabled); }} 
                       className={`w-14 h-7 rounded-full relative cursor-pointer transition-all ${vfxEnabled ? 'bg-cyan-500 shadow-[0_0_15px_cyan]' : 'bg-white/20'}`}
                     >
                       <div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${vfxEnabled ? 'right-1' : 'left-1'}`}></div>
                     </div>
                   </div>
               ...
               <div className="flex items-center justify-between">
                 <div className="cursor-default" onMouseEnter={() => playSFX('hover')}>
                   <h4 className="text-xl font-black text-pink-400 tracking-widest">디스코드 연동 동기화</h4>
                   <p className="text-sm text-slate-400 font-bold mt-2">디스코드 닉네임과 프로필 사진을 강제로 다시 불러옵니다.</p>
                 </div>
                 <button onMouseEnter={() => playSFX('hover')} onClick={() => playSFX('click')} className="px-8 py-3 bg-white/10 border-2 border-pink-500/50 hover:bg-pink-500 text-pink-400 hover:text-white rounded-full font-black tracking-widest transition-all cursor-pointer">
                   SYNC NOW
                 </button>
               </div>

               {/* ✨ CREATED BY CONYMON 워터마크 */}
               <div className="mt-12 pt-8 flex flex-col items-center justify-center opacity-80 cursor-default pointer-events-none">
                 <p className="text-[10px] text-slate-500 font-black tracking-[0.5em] mb-2 uppercase">System Architect & Designed By</p>
                 <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 tracking-tighter drop-shadow-[0_0_15px_rgba(34,211,238,0.5)] italic">
                   CONYMON
                 </h1>
                 <p className="text-[8px] text-slate-600 tracking-widest mt-2">© 2026 GALAXY THRONE. ALL RIGHTS RESERVED.</p>
               </div>
      ...
      {selectedPlayer && (
        <div 
          className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-6 animate-in fade-in duration-300 cursor-pointer"
          onClick={() => { playSFX('click'); setSelectedPlayer(null); }}
        >
          <div 
            className="bg-[#0A0C14] border-2 border-cyan-400 w-full max-w-2xl rounded-[4rem] p-12 shadow-[0_0_50px_rgba(34,211,238,0.3)] relative overflow-hidden cursor-default"
          ...
          >
             ...
             <div className="flex items-center gap-10 mb-10 relative z-10 cursor-default">
                ...
                <div className="flex-1 overflow-hidden">
                   ...
                   <div className="flex gap-3">
                     <span className={`text-sm font-black px-6 py-1.5 rounded-full border border-white/20 inline-block text-left ${getGrandRankInfo(selectedPlayer.rankIndex)?.color || 'text-slate-300'} bg-white/10 uppercase tracking-widest`}>
                        RANK: {getGrandRankInfo(selectedPlayer.rankIndex)?.title} {getGrandRankInfo(selectedPlayer.rankIndex)?.num}
                     </span>
                     <span className="text-sm font-black px-6 py-1.5 rounded-full border border-white/10 inline-block text-slate-400 bg-white/5 tracking-widest select-text cursor-text">
                        @{selectedPlayer.discord_id}
                     </span>
                   </div>
                </div>
             </div>
             ...
             <div className="mb-6 relative z-10 flex justify-center cursor-default" onMouseEnter={() => playSFX('hover')}>
                 <div className={`px-8 py-3 rounded-full border-2 font-black tracking-widest text-lg shadow-lg ${selectedPlayer.rankIndex === 0 ? 'bg-yellow-400/10 border-yellow-400 text-yellow-400 shadow-[0_0_15px_gold]' : 'bg-slate-800/50 border-slate-600 text-slate-300'}`}>
                    {selectedPlayer.rankIndex === 0 ? '👑 왕좌 방어전 스택 : ' : '🛡️ 방어전 스택 : '} {selectedPlayer.defense_stack || 0}
                 </div>
             </div>

             <div className="grid grid-cols-2 gap-6 mb-10 relative z-10 cursor-default">
                <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center shadow-inner flex flex-col justify-center hover:border-cyan-400/50 transition-all" onMouseEnter={() => playSFX('hover')}>
                   <p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">Match Record (W/L)</p>
                   <p className="text-4xl font-black text-white">{selectedPlayer.wins}승 {selectedPlayer.losses}패</p>
                </div>
                <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center shadow-inner flex flex-col justify-center hover:border-cyan-400/50 transition-all" onMouseEnter={() => playSFX('hover')}>
                   <p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">Win Rate</p>
                   <p className="text-4xl font-black text-pink-400">{selectedPlayer.win_rate || "0.0%"}</p>
                </div>
                <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center shadow-inner flex flex-col justify-center hover:border-cyan-400/50 transition-all" onMouseEnter={() => playSFX('hover')}>
                   <p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">Favorite Legend</p>
                   <p className="text-2xl font-black text-cyan-400 truncate px-2">{selectedPlayer.favorite_legend || "방갈로르"}</p>
                </div>
                <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center shadow-inner flex flex-col justify-center hover:border-cyan-400/50 transition-all" onMouseEnter={() => playSFX('hover')}>
                   <p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">Favorite Weapon</p>
                   <p className="text-xl font-black text-cyan-400 truncate px-2">{selectedPlayer.favorite_weapon || "VK-47 플랫라인"}</p>
                </div>
             </div>

             <button onMouseEnter={() => playSFX('hover')} onClick={() => copyDiscordId(selectedPlayer.discord_id)} className="w-full bg-cyan-600/20 hover:bg-cyan-600/40 border-2 border-cyan-400/50 text-cyan-400 py-6 rounded-[2rem] font-black text-lg flex items-center justify-center gap-4 transition-all active:scale-95 shadow-[0_0_20px_rgba(34,211,238,0.2)] relative z-10 cursor-pointer">
                {copyStatus ? <Check size={24}/> : <Copy size={24}/>}{copyStatus ? 'ID COPIED!' : `COPY DISCORD ID`}
             </button>
             <div className="absolute -top-32 -right-32 w-96 h-96 bg-cyan-500/10 rounded-full blur-[100px] z-0 pointer-events-none"></div>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        .comet-path { position: absolute; width: 250%; height: 100%; top: -30%; left: -120%; transform: rotate(12deg); animation: comet-move 2.8s ease-in-out forwards; }
        .comet-head { position: absolute; width: 8px; height: 8px; background: #fff; border-radius: 50%; top: 50%; right: 0; box-shadow: 0 0 30px 5px #fff; }
        .comet-tail { position: absolute; width: 800px; height: 4px; background: linear-gradient(to left, rgba(255,255,255,0.9), transparent); top: 52%; right: 4px; filter: blur(1px); }
        @keyframes comet-move { 0% { left: -120%; opacity: 0; } 15% { opacity: 1; } 85% { opacity: 1; } 100% { left: 120%; opacity: 0; } }
        optgroup { font-style: normal; font-weight: 900; background: #000; color: #fff; }
        option { background-color: #000; color: #fff; padding: 10px; }
        
        .combat-log-item { padding: 0.5rem 0.75rem; }
        .combat-log-item .nickname { font-size: 1.15rem; }
        .combat-log-item .details { font-size: 0.95rem; }
        
        .rank-item { padding: 0.5rem 0.75rem; }
        .rank-item .nickname { font-size: 1.25rem; }
