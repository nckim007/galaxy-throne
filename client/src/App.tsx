...
const BASE_VOLUMES: Record<string, number> = {
  hover: 0.5, click: 1.0, match_start: 1.0, success: 1.0, error: 1.0         
};
...
if (typeof window !== 'undefined') {
  ...
  const unlockAudio = () => {
    ...
    Object.keys(audioCache).forEach(key => {
      ...
      audio.muted = false;
      if (key === 'bgm' && currentMatchPhase === 'idle' && globalBgmEnabled) {
        audio.play().catch(() => {});
      } else {
        const p = audio.play();
        if (p !== undefined) { p.then(() => { audio.pause(); audio.currentTime = 0; }).catch(() => {}); }
      }
    });
...
function App() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('challenges_realtime')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'challenges' }, payload => {
        ...
        // 상대방이 나를 지목했는지 확인
        if (newChallenge.challenger_name === entryOpponent && newChallenge.target_name === currentUserName) {
          playSFX('match_start');
          setActiveMatch({
            id: newChallenge.id,
            mode: newChallenge.mode,
            opponent: entryOpponent,
            legend: entryMode === 'random' ? newChallenge.legend : entryLegend,
            weapons: entryMode === 'random' ? newChallenge.weapons : entryWeapons
          });
          setMatchPhase('ready');
        }
      })
  ...
  }, [matchPhase, entryOpponent, currentUserName]);
  ...
  const handleStartMatch = async () => {
    ...
    // 🌟 쌍방 확인 로직: 상대방이 이미 나를 기다리고 있는지 체크
    const { data: existing } = await supabase.from('challenges')
      .select('*')
      .eq('challenger_name', entryOpponent)
      .eq('target_name', currentUserName)
      .eq('mode', entryMode)
      .single();
    ...
    if (existing) {
      ...
      setActiveMatch({
        ...
        mode: entryMode,
        opponent: entryOpponent,
        legend: entryMode === 'random' ? existing.legend : entryLegend,
        weapons: entryMode === 'random' ? existing.weapons : entryWeapons
      });
      setMatchPhase('ready');
    } else {
      // 대기열 등록
      ...
      await supabase.from('challenges').insert([{
        ...
        target_name: entryOpponent,
        mode: entryMode,
        legend: entryMode === 'random' ? randL : entryLegend,
        weapons: entryMode === 'random' ? randW : entryWeapons
      }]);
  ...
  const handleCancelMatch = async () => {
    playSFX('click');
    await supabase.from('challenges').delete().eq('challenger_name', currentUserName);
    setMatchPhase('idle');
    setActiveMatch(null);
  };
  ...
  const handleReportScore = async () => {
    ...
    if (!error) { 
      await supabase.from('challenges').delete().eq('challenger_name', currentUserName);
      await supabase.from('challenges').delete().eq('target_name', currentUserName);
      alert("전투 보고 완료!"); 
  ...
  const getGrandRankInfo = (idx: number) => {
    ...
    if (idx < 6) return { title: "성좌", num: idx + 1, color: "text-purple-400", glow: "shadow-[0_0_10px_rgba(192,132,252,0.6)]", bg: "bg-purple-400/20", icon: <Star size={12} className="text-purple-400"/> };
    ...
    return (
      <div key={index} onMouseEnter={() => playSFX('hover')} className="bg-black/60 border border-cyan-500/30 p-5 rounded-[1.5rem] flex flex-col justify-center gap-3 hover:border-cyan-400/80 transition-all shadow-inner relative group combat-log-item cursor-default h-[140px] shrink-0">
        ...
        <div className="flex items-center justify-between w-full mt-2">
          ...
          <div className="text-3xl font-black tracking-widest text-white shrink-0 ml-2 drop-shadow-[0_0_10px_white]"><span className="text-yellow-400">{winnerScore}</span><span className="text-slate-600 mx-1">:</span><span className="text-slate-400">{loserScore}</span></div>
        </div>
        <div className="flex items-center gap-2 text-xs font-black bg-white/5 px-4 py-2 rounded-xl w-fit shadow-inner border border-white/10 details mt-1"><span className="text-pink-400">[{usedLegend}]</span><span className="text-cyan-300">{weapon1}</span><span className="text-slate-500">/</span><span className="text-cyan-300">{weapon2}</span></div>
      </div>
    );
  };
    ...
    <div className="flex h-screen bg-black text-slate-300 font-sans overflow-hidden relative select-none">
      ...
      <div className="flex-1 flex flex-col z-10 relative ml-20 h-screen overflow-y-auto custom-scrollbar">
        <header className="px-10 py-8 flex justify-between items-center shrink-0 border-b border-cyan-500/30 bg-black/20 backdrop-blur-md shadow-[0_0_30px_rgba(34,211,238,0.15)]">
          <div className="flex items-center gap-6 cursor-pointer" onMouseEnter={() => playSFX('hover')} onClick={() => playSFX('click')}><h1 className="text-6xl font-black text-white italic tracking-tighter uppercase drop-shadow-[0_0_20px_rgba(192,132,252,0.8)]">은하단</h1><div className="h-12 w-[3px] bg-gradient-to-b from-cyan-400 to-purple-400 mx-3 shadow-[0_0_10px_cyan]"></div><div className="flex flex-col justify-center"><p className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 uppercase tracking-widest drop-shadow-[0_0_8px_rgba(34,211,238,0.4)]">별의 전쟁 : 시즌 1</p><p className="text-[12px] font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-purple-300 tracking-[0.4em] mt-1.5 uppercase italic">SEASON 01 BATTLE FOR THE STAR THRONE</p></div></div>
          {user ? (
            <div onMouseEnter={() => playSFX('hover')} onClick={handleLogout} className="flex items-center gap-4 bg-black/60 p-2 rounded-full border border-white/10 pr-8 shadow-2xl border-l-cyan-500 border-l-4 cursor-pointer hover:border-l-pink-500 transition-all group"><img src={currentUserAvatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=Guest"} className="w-10 h-10 rounded-full border border-cyan-500/50 group-hover:border-pink-500" alt="profile"/><div className="flex flex-col"><span className="text-sm font-black text-white">{currentUserName || "GUEST"}</span><span className="text-[9px] font-bold text-cyan-400 group-hover:text-pink-400 uppercase tracking-tighter">Click to Logout</span></div></div>
          ...
          <main className="flex-1 p-10 grid grid-cols-12 gap-8 items-stretch pb-20 animate-in fade-in duration-500 min-h-[1400px]">
            <div className="col-span-12 xl:col-span-3 flex flex-col gap-8 h-full relative">
               <section className="bg-black/40 backdrop-blur-2xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[2.5rem] p-3 shrink-0 h-fit flex flex-col overflow-hidden relative">
                  <div className="flex flex-col relative z-10"><h3 className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 uppercase tracking-tight text-center w-full block mb-2 border-b border-white/5 pb-2 shrink-0 drop-shadow-[0_0_8px_cyan]">매치 엔트리</h3>
                      <div className="space-y-3 flex flex-col relative pb-1">
                          ...
                          <div className="bg-black/60 p-2.5 rounded-2xl border border-white/5 shadow-inner shrink-0 relative z-20"><p className="text-[9px] text-slate-500 font-black uppercase text-left mb-1 tracking-widest">Target Name</p><input value={entryOpponent} onChange={(e) => setEntryOpponent(e.target.value)} placeholder="상대방 닉네임" className="w-full bg-transparent outline-none text-white font-black text-lg px-1 select-text cursor-text" /></div>
                          {entryMode === 'free' && (
                            <div className="space-y-2 flex flex-col relative z-10 overflow-hidden">
                               <select onMouseEnter={() => playSFX('hover')} value={entryLegend} onChange={(e) => { playSFX('click'); setEntryLegend(e.target.value); }} className={`w-full bg-black/60 border border-white/10 p-2.5 rounded-xl text-sm font-black outline-none cursor-pointer shrink-0 select-text ${entryLegend === '' ? 'text-slate-500' : 'text-white'}`}><option value="" disabled hidden>👉 사용하실 레전드를 선택하세요</option>{Object.entries(LEGEND_CATEGORIES).map(([cat, list]) => (<optgroup key={cat} label={cat} className="bg-black text-cyan-400 font-black">{list.map(l => <option key={l} value={l} className="text-white">{l}</option>)}</optgroup>))}</select>
                               <div className="flex flex-col gap-2 overflow-hidden relative">{[0, 1].map(i => (<select key={i} onMouseEnter={() => playSFX('hover')} value={entryWeapons[i]} onChange={(e) => {const w = [...entryWeapons]; w[i] = e.target.value; setEntryWeapons(w); playSFX('click');}} className={`w-full bg-black/60 border border-white/10 p-2.5 rounded-xl text-sm font-black outline-none cursor-pointer shrink-0 select-text ${entryWeapons[i] === '' ? 'text-slate-500' : 'text-pink-400'}`}><option value="" disabled hidden>👉 {i+1}번 무기를 선택하세요</option>{Object.entries(WEAPON_CATEGORIES).map(([cat, list]) => (<optgroup key={cat} label={cat} className="bg-black text-pink-400 font-black">{list.map(w => <option key={w} value={w} className="text-white">{w}</option>)}</optgroup>))}</select>))}</div>
                            </div>
                          )}
               ...
               <section className="bg-black/50 backdrop-blur-2xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[2.5rem] p-4 flex flex-col overflow-hidden h-[880px] shrink-0">
                  <h3 className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-cyan-400 uppercase tracking-tight text-center w-full block mb-2 border-b border-white/5 pb-2 shrink-0 flex items-center justify-center gap-2 drop-shadow-[0_0_8px_cyan] cursor-default" onMouseEnter={() => playSFX('hover')}><span className="w-2.5 h-2.5 bg-green-500 rounded-full shadow-[0_0_10px_#22c55e] animate-pulse"></span> 접속 현황</h3>
                     ...
                     {rankers.length > 0 ? rankers.map((ou, i) => {
                        ...
                        return (
                          <div key={i} onMouseEnter={() => playSFX('hover')} className="bg-black/60 border border-white/10 p-3 rounded-xl flex items-center justify-between hover:border-cyan-400/50 transition-all group cursor-default">
                             <div className="flex items-center gap-3"><div className="relative"><img src={ou.avatar_url} className="w-9 h-9 rounded-full border border-white/20" alt="profile"/><div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-black rounded-full"></div></div><div className="flex flex-col"><span className="text-sm font-black text-white truncate max-w-[90px]">{ou.display_name}</span><span className={`text-[9px] font-black uppercase tracking-widest ${rankInfo.color}`}>{rankInfo.title} {rankInfo.num}</span></div></div>
                             <button onClick={() => { playSFX('click'); setEntryOpponent(ou.display_name); }} disabled={!isChallengeableByRank} className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all shadow-[0_0_10px_rgba(34,211,238,0.2)] cursor-pointer ${isChallengeableByRank ? 'bg-cyan-600/20 border border-cyan-500/50 text-cyan-400 hover:bg-cyan-500 hover:text-black' : 'bg-slate-800/50 border border-slate-700 text-slate-500 cursor-not-allowed'}`}>{isChallengeableByRank ? '도전!' : '하위 랭크'}</button>
                          </div>
                        )
            ...
            <div className="col-span-12 xl:col-span-5 flex flex-col gap-8 h-full relative">
               <section className="bg-black/50 backdrop-blur-3xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[3rem] p-4 overflow-hidden flex flex-col justify-between h-fit shrink-0 relative">
                  <div className="flex flex-col relative z-10"><h3 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 uppercase tracking-tight text-center w-full block mb-2 border-b border-white/5 pb-2 shrink-0 mt-1 drop-shadow-[0_0_8px_cyan]">현재 매치 상황</h3>
                      {matchPhase === 'waiting' ? (
                        <div className="flex flex-col items-center justify-center opacity-90 relative z-10 py-16 animate-in fade-in"><div className="w-20 h-20 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin mb-8 shadow-[0_0_15px_cyan]"></div><h4 className="text-3xl font-black text-cyan-400 tracking-widest drop-shadow-[0_0_10px_cyan]">상대 대기 중</h4><p className="text-sm font-bold text-slate-300 mt-4 tracking-widest text-center">상대방(<span className="text-pink-400">{entryOpponent}</span>)이 당신의 닉네임을 입력하고<br/>대전 생성을 누르면 즉시 시작됩니다.</p><button onMouseEnter={() => playSFX('hover')} onClick={handleCancelMatch} className="mt-10 px-10 py-3 rounded-full border-2 border-pink-500/50 text-pink-400 font-black hover:bg-pink-500 hover:text-white transition-all shadow-[0_0_15px_rgba(236,72,153,0.3)] cursor-pointer">매칭 취소 (Cancel)</button></div>
                      ) : matchPhase === 'idle' || !activeMatch ? (<div className="flex flex-col items-center justify-center opacity-10 relative z-10 py-20 cursor-default" onMouseEnter={() => playSFX('hover')}><Shield size={100}/><p className="text-xl font-black uppercase tracking-[0.4em] mt-8 drop-shadow-[0_0_10px_white]">Ready for Combat</p></div>
                      ) : (
                        <div className="flex flex-col pt-1 pb-1 animate-in fade-in overflow-hidden relative z-10 gap-3 mt-1"><div className={`p-6 rounded-[2.5rem] border-2 shadow-[0_0_40px_rgba(0,0,0,0.5)] ${activeMatch.mode === 'random' ? 'border-cyan-400/50 bg-cyan-400/5' : 'border-pink-400/50 bg-pink-400/5'} text-center relative overflow-hidden flex flex-col justify-center shrink-0 cursor-default`} onMouseEnter={() => playSFX('hover')}><h4 className="text-6xl font-black text-white tracking-tighter mb-6 drop-shadow-[0_0_20px_white] leading-tight shrink-0">{activeMatch.legend}</h4><div className="flex flex-col gap-3 items-center shrink-0">{activeMatch.weapons.map((w:any, idx:number) => (<span key={idx} className={`px-6 py-2 rounded-2xl text-base font-black border-2 w-full max-w-[350px] shadow-inner ${activeMatch.mode === 'random' ? 'border-cyan-400/50 text-cyan-400' : 'border-pink-400/50 text-pink-400'}`}>{w}</span>))}</div></div><div className="text-center bg-black/40 p-4 rounded-[1.5rem] border border-white/10 shadow-inner shrink-0 relative z-20 cursor-default" onMouseEnter={() => playSFX('hover')}><p className="text-[10px] text-slate-500 font-black uppercase text-center mb-1 tracking-widest">Target Opponent</p><h4 className="text-5xl font-black text-white italic truncate px-2 pb-2 pt-1 drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">{activeMatch.opponent}</h4></div><div className="flex flex-col gap-3 pt-3 border-t border-white/5 shrink-0 relative z-20"><div className="flex justify-center gap-4 shrink-0">{[0, 1, 2, 3].map(num => (<button key={num} onMouseEnter={() => playSFX('hover')} onClick={() => { playSFX('click'); setMyReportedScore(num); }} className={`w-14 h-14 rounded-3xl font-black text-2xl transition-all border-2 cursor-pointer ${myReportedScore === num ? 'bg-cyan-500 border-white text-black scale-110 shadow-[0_0_20px_cyan]' : 'bg-black/60 border-white/10 text-white hover:bg-white/5'}`}>{num}</button>))}</div><button onMouseEnter={() => playSFX('hover')} onClick={handleReportScore} disabled={myReportedScore === null} className={`w-full py-4 rounded-[1.5rem] font-black text-xl transition-all uppercase tracking-widest shrink-0 relative z-20 cursor-pointer ${myReportedScore !== null ? 'text-cyan-400 border-2 border-cyan-400/50 shadow-[0_0_15px_rgba(34,211,238,0.3)] hover:bg-cyan-400 hover:text-black' : 'text-slate-500 border-2 border-slate-700 bg-slate-800/50 cursor-not-allowed'}`}>{myReportedScore !== null ? '전투 결과 입력' : '스코어를 선택하세요'}</button></div></div>
                      )}
                  </div></section>
               <section className="bg-black/40 backdrop-blur-2xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[2.5rem] p-6 flex flex-col overflow-hidden h-[940px] shrink-0"><h3 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 uppercase tracking-tight text-center w-full block mb-4 border-b border-white/5 pb-2 shrink-0 drop-shadow-[0_0_8px_cyan] cursor-default" onMouseEnter={() => playSFX('hover')}>최근 전투 기록</h3><div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2 pb-4">{logs.length > 0 ? logs.slice(0, 5).map((log, i) => renderCombatLogItem(log, i)) : (<div className="flex items-center justify-center h-full opacity-50 text-sm font-bold tracking-widest text-cyan-400">아직 진행된 전투가 없습니다</div>)}</div></section>
            </div>
            <div className="col-span-12 lg:col-span-4 flex flex-col h-full relative">
               <section className="bg-black/40 backdrop-blur-3xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[3.5rem] p-5 flex flex-col overflow-hidden h-fit shrink-0 relative"><div className="px-2 pt-2 flex flex-col relative z-10"><div className="flex flex-col items-start shrink-0 mb-4 mt-1"><div className="flex items-center gap-6 mb-1 cursor-default" onMouseEnter={() => playSFX('hover')}><Trophy className="text-yellow-400 shrink-0 drop-shadow-[0_0_15px_rgba(250,204,21,0.5)]" size={80}/><div className="flex flex-col"><h3 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-pink-500 uppercase tracking-tighter drop-shadow-[0_0_10px_orange] mt-1 pt-1">은하단 랭킹</h3><p className="text-sm font-black text-cyan-400 italic uppercase leading-tight pt-1 drop-shadow-[0_0_8px_cyan]">명예의 전당<br/><span className="text-xs text-slate-400">(HALL OF FAME)</span></p></div></div></div>
                         ...
                         {rankers.length > 0 ? rankers.filter(r => r.display_name?.includes(searchQuery)).filter(r => { if (rankTab === 0) return r.rankIndex < 6; if (rankTab === 1) return r.rankIndex >= 6 && r.rankIndex < 12; return r.rankIndex >= 12; }).map((r) => {
                              ...
                              return (
                                <div key={r.id} onMouseEnter={() => playSFX('hover')} onClick={() => { playSFX('click'); setSelectedPlayer({...r, rankIndex: r.rankIndex}); }} className={`mx-1 p-3 pt-8 rounded-[1.2rem] border transition-all cursor-pointer group hover:scale-[1.02] ${grandRank.glow} border-cyan-400/30 hover:border-cyan-400 bg-black/60 relative flex flex-col justify-center shrink-0 z-20 rank-item`}>
                                   {r.rankIndex === 0 && <div className="absolute inset-0 bg-yellow-400/5 animate-pulse rounded-[1.2rem] pointer-events-none"></div>}<div className={`absolute top-0 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-b-lg border-x border-b border-cyan-400/30 ${grandRank.bg} flex items-center gap-1 shadow-md`}>{grandRank.icon} <span className={`text-[10px] sm:text-[11px] font-black uppercase tracking-wider ${grandRank.color}`}>{grandRank.title} {grandRank.num}</span></div><div className="flex items-center justify-between w-full mt-3 relative z-10 px-2"><div className="flex items-center gap-4 flex-1 overflow-hidden"><img src={r.avatar_url} className={`w-10 h-10 rounded-full border-2 ${r.rankIndex === 0 ? 'border-yellow-400 shadow-[0_0_10px_gold]' : 'border-white/20'} shrink-0`} alt="profile"/><span className={`text-white font-black truncate group-hover:text-cyan-400 transition-colors tracking-tight whitespace-nowrap nickname ${r.rankIndex === 0 ? 'text-2xl' : 'text-xl'}`}>{r.display_name}</span></div><div className="flex flex-col items-end gap-1 shrink-0 ml-2"><span className="font-black text-slate-300 text-lg tracking-widest">{r.wins}승 {r.losses}패</span>{r.rankIndex === 0 && (<span className={`font-black text-[10px] px-2 py-0.5 rounded bg-yellow-400/20 text-yellow-400 border border-yellow-400/50 shadow-[0_0_5px_gold]`}>👑 왕좌방어 스택: {r.defense_stack || 0}</span>)}</div></div>
                                </div>
                              );
                           }) : (<div className="flex flex-col items-center justify-center h-[500px] opacity-50 text-sm font-bold tracking-widest mt-10 text-cyan-400 gap-4"><Crown size={48} className="text-cyan-400 mb-2 opacity-50"/>아직 등록된 랭커가 없습니다<br/>디스코드로 로그인하여 최초의 왕좌를 차지하세요!</div>)}
                      </div>
                      <div className="flex gap-2 shrink-0 relative z-20 px-1 mt-1 mb-1"><button onMouseEnter={() => playSFX('hover')} onClick={() => { playSFX('click'); setRankTab(0); }} className={`flex-1 py-3 rounded-xl text-xs font-black transition-all border cursor-pointer ${rankTab === 0 ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/50 shadow-[0_0_10px_rgba(250,204,21,0.3)]' : 'bg-black/40 border-white/10 text-slate-500 hover:text-white hover:border-cyan-400/50'}`}>왕좌·성좌</button><button onMouseEnter={() => playSFX('hover')} onClick={() => { playSFX('click'); setRankTab(1); }} className={`flex-1 py-3 rounded-xl text-xs font-black transition-all border cursor-pointer ${rankTab === 1 ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50 shadow-[0_0_10px_rgba(34,211,238,0.3)]' : 'bg-black/40 border-white/10 text-slate-500 hover:text-white hover:border-cyan-400/50'}`}>항성 (1~6)</button><button onMouseEnter={() => playSFX('hover')} onClick={() => { playSFX('click'); setRankTab(2); }} className={`flex-1 py-3 rounded-xl text-xs font-black transition-all border cursor-pointer ${rankTab === 2 ? 'bg-slate-500/20 text-slate-300 border-slate-500/50 shadow-[0_0_10px_rgba(148,163,184,0.3)]' : 'bg-black/40 border-white/10 text-slate-500 hover:text-white hover:border-cyan-400/50'}`}>정예</button></div>
                      <div className="relative shrink-0 px-1 relative z-20 mt-3 mb-1"><Search className="absolute left-6 top-4.5 text-slate-500" size={18}/><input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="검색..." className="w-full bg-white/5 border border-white/10 pl-16 pr-8 py-4 rounded-full text-sm outline-none focus:border-cyan-400 text-white transition-all shadow-inner relative z-20 select-text cursor-text"/></div>
                  </div></section>
            </div>
          </main>
        )}
        ...
        {activeMenu === 'settings' && (
          <main className="flex-1 p-10 h-full overflow-y-auto custom-scrollbar pb-20 animate-in fade-in duration-500"><h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-slate-300 to-slate-500 uppercase tracking-widest mb-8 text-left flex items-center gap-4 drop-shadow-[0_0_10px_white]"><Settings size={36} className="text-slate-400"/> 환경 설정 (Settings)</h2><div className="bg-black/50 backdrop-blur-2xl border-2 border-cyan-400 shadow-[0_0_25px_rgba(34,211,238,0.5)] rounded-[3rem] p-12 max-w-3xl space-y-10"><div className="flex items-center gap-8 border-b border-white/10 pb-10 cursor-default" onMouseEnter={() => playSFX('hover')}><img src={currentUserAvatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=Guest&backgroundColor=b6e3f4"} className="w-24 h-24 rounded-full border-4 border-cyan-400 shadow-[0_0_20px_cyan]" alt="profile"/><div><h3 className="text-3xl font-black text-white italic tracking-tighter uppercase">{currentUserName || "GUEST PILOT"}</h3><p className="text-cyan-400 font-bold tracking-widest mt-1">{user?.email || "디스코드 로그인이 필요합니다"}</p></div></div><div className="flex items-center justify-between border-b border-white/10 pb-8"><div className="flex-1"><div className="flex justify-between items-center mb-2"><h4 className="text-xl font-black text-white tracking-widest cursor-default" onMouseEnter={() => playSFX('hover')}>배경음악 (BGM)</h4><div onClick={() => { playSFX('click'); setBgmEnabled(!bgmEnabled); }} className={`w-14 h-7 rounded-full relative cursor-pointer transition-all ${bgmEnabled ? 'bg-cyan-500 shadow-[0_0_15px_cyan]' : 'bg-white/20'}`}><div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${bgmEnabled ? 'right-1' : 'left-1'}`}></div></div></div><p className="text-sm text-slate-400 font-bold mb-4 cursor-default">로비 및 대기 화면의 배경음악을 재생합니다.</p><div className="flex items-center gap-4"><span className="text-xs font-black text-slate-500">0</span><input type="range" min="0" max="1" step="0.01" value={bgmVolume} onChange={(e) => setBgmVolume(parseFloat(e.target.value))} className="w-full bg-white/10 h-2 rounded-lg cursor-pointer volume-slider" disabled={!bgmEnabled} onMouseEnter={() => playSFX('hover')}/><span className="text-xs font-black text-cyan-400 w-8 text-right">{Math.round(bgmVolume * 100)}</span></div></div></div><div className="flex items-center justify-between border-b border-white/10 pb-8"><div className="flex-1"><div className="flex justify-between items-center mb-2"><h4 className="text-xl font-black text-white tracking-widest cursor-default" onMouseEnter={() => playSFX('hover')}>효과음 (SFX)</h4><div onClick={() => { playSFX('click'); setSfxEnabled(!sfxEnabled); }} className={`w-14 h-7 rounded-full relative cursor-pointer transition-all ${sfxEnabled ? 'bg-cyan-500 shadow-[0_0_15px_cyan]' : 'bg-white/20'}`}><div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${sfxEnabled ? 'right-1' : 'left-1'}`}></div></div></div><p className="text-sm text-slate-400 font-bold mb-4 cursor-default">버튼 클릭 및 매칭 시 효과음을 재생합니다.</p><div className="flex items-center gap-4"><span className="text-xs font-black text-slate-500">0</span><input type="range" min="0" max="1" step="0.01" value={sfxVolume} onChange={(e) => setSfxVolume(parseFloat(e.target.value))} className="w-full bg-white/10 h-2 rounded-lg cursor-pointer volume-slider" disabled={!sfxEnabled} onMouseEnter={() => playSFX('hover')}/><span className="text-xs font-black text-cyan-400 w-8 text-right">{Math.round(sfxVolume * 100)}</span></div></div></div><div className="flex items-center justify-between border-b border-white/10 pb-8"><div className="flex-1"><div className="flex justify-between items-center mb-2"><h4 className="text-xl font-black text-white tracking-widest cursor-default" onMouseEnter={() => playSFX('hover')}>배경 애니메이션 (VFX)</h4><div onClick={() => { playSFX('click'); setVfxEnabled(!vfxEnabled); }} className={`w-14 h-7 rounded-full relative cursor-pointer transition-all ${vfxEnabled ? 'bg-cyan-500 shadow-[0_0_15px_cyan]' : 'bg-white/20'}`}><div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${vfxEnabled ? 'right-1' : 'left-1'}`}></div></div></div><p className="text-sm text-slate-400 font-bold mt-2 cursor-default">1위 유저 변경 시 나타나는 우주 혜성 이펙트를 표시합니다.</p></div></div><div className="flex items-center justify-between"><div className="cursor-default" onMouseEnter={() => playSFX('hover')}><h4 className="text-xl font-black text-pink-400 tracking-widest">디스코드 연동 동기화</h4><p className="text-sm text-slate-400 font-bold mt-2">디스코드 닉네임과 프로필 사진을 강제로 다시 불러옵니다.</p></div><button onMouseEnter={() => playSFX('hover')} onClick={() => playSFX('click')} className="px-8 py-3 bg-white/10 border-2 border-pink-500/50 hover:bg-pink-500 text-pink-400 hover:text-white rounded-full font-black tracking-widest transition-all cursor-pointer">SYNC NOW</button></div><div className="mt-12 pt-8 flex flex-col items-center justify-center opacity-80 cursor-default pointer-events-none"><p className="text-[10px] text-slate-500 font-black tracking-[0.5em] mb-2 uppercase">System Architect & Designed By</p><h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 tracking-tighter drop-shadow-[0_0_15px_rgba(34,211,238,0.5)] italic">CONYMON</h1><p className="text-[8px] text-slate-600 tracking-widest mt-2">© 2026 GALAXY THRONE. ALL RIGHTS RESERVED.</p></div></div></main>
        )}
      </div>
      ...
      {selectedPlayer && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-6 animate-in fade-in duration-300 cursor-pointer" onClick={() => { playSFX('click'); setSelectedPlayer(null); }}>
          <div className="bg-[#0A0C14] border-2 border-cyan-400 w-full max-w-2xl rounded-[4rem] p-12 shadow-[0_0_50px_rgba(34,211,238,0.3)] relative overflow-hidden cursor-default" onClick={(e) => e.stopPropagation()} onMouseEnter={() => playSFX('hover')}>
             <button onMouseEnter={() => playSFX('hover')} onClick={() => { playSFX('click'); setSelectedPlayer(null); }} className="absolute top-10 right-10 text-slate-500 hover:text-white transition-colors cursor-pointer z-50"><X size={36}/></button><div className="flex items-center gap-10 mb-10 relative z-10 cursor-default"><img src={selectedPlayer.avatar_url} className={`w-32 h-32 rounded-[2.5rem] border-4 ${selectedPlayer.rankIndex === 0 ? 'border-yellow-400 shadow-[0_0_30px_gold]' : 'border-cyan-400 shadow-[0_0_20px_cyan]'} shrink-0`} alt="profile" /><div className="flex-1 overflow-hidden"><h2 className="text-5xl font-black text-white italic uppercase truncate w-full tracking-tighter text-left mb-2">{selectedPlayer.display_name}</h2><div className="flex gap-3"><span className={`text-sm font-black px-6 py-1.5 rounded-full border border-white/20 inline-block text-left ${getGrandRankInfo(selectedPlayer.rankIndex)?.color || 'text-slate-300'} bg-white/10 uppercase tracking-widest`}>RANK: {getGrandRankInfo(selectedPlayer.rankIndex)?.title} {getGrandRankInfo(selectedPlayer.rankIndex)?.num}</span></div></div></div>
             <div className="mb-6 relative z-10 flex justify-center cursor-default" onMouseEnter={() => playSFX('hover')}><div className={`px-8 py-3 rounded-full border-2 font-black tracking-widest text-lg shadow-lg ${selectedPlayer.rankIndex === 0 ? 'bg-yellow-400/10 border-yellow-400 text-yellow-400 shadow-[0_0_15px_gold]' : 'bg-slate-800/50 border-slate-600 text-slate-300'}`}>{selectedPlayer.rankIndex === 0 ? '👑 왕좌 방어전 스택 : ' : '🛡️ 방어전 스택 : '} {selectedPlayer.defense_stack || 0}</div></div>
             <div className="grid grid-cols-2 gap-6 mb-10 relative z-10 cursor-default"><div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center shadow-inner flex flex-col justify-center hover:border-cyan-400/50 transition-all" onMouseEnter={() => playSFX('hover')}><p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">Match Record (W/L)</p><p className="text-4xl font-black text-white">{selectedPlayer.wins}승 {selectedPlayer.losses}패</p></div><div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center shadow-inner flex flex-col justify-center hover:border-cyan-400/50 transition-all" onMouseEnter={() => playSFX('hover')}><p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">Win Rate</p><p className="text-4xl font-black text-pink-400">{selectedPlayer.win_rate || "0.0%"}</p></div><div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center shadow-inner flex flex-col justify-center hover:border-cyan-400/50 transition-all" onMouseEnter={() => playSFX('hover')}><p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">Favorite Legend</p><p className="text-2xl font-black text-cyan-400 truncate px-2">{selectedPlayer.favorite_legend || "미선택"}</p></div><div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center shadow-inner flex flex-col justify-center hover:border-cyan-400/50 transition-all" onMouseEnter={() => playSFX('hover')}><p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">Favorite Weapon</p><p className="text-xl font-black text-cyan-400 truncate px-2">{selectedPlayer.favorite_weapon || "미선택"}</p></div></div>
             <button onMouseEnter={() => playSFX('hover')} onClick={() => copyPlayerName(selectedPlayer.display_name)} className="w-full bg-cyan-600/20 hover:bg-cyan-600/40 border-2 border-cyan-400/50 text-cyan-400 py-6 rounded-[2rem] font-black text-lg flex items-center justify-center gap-4 transition-all active:scale-95 shadow-[0_0_20px_rgba(34,211,238,0.2)] relative z-10 cursor-pointer">{copyStatus ? <Check size={24}/> : <Copy size={24}/>}{copyStatus ? 'NICKNAME COPIED!' : `COPY NICKNAME`}</button>
          </div>
        </div>
      )}
